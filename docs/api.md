...
服务器文件、临时 URL、配置应用和强制更新 POST 请求体上限为 3MiB，path `:id` 必须是 36 字节 canonical UUID 文本；非法、simple 或大写 UUID 会在 Agent 可见性校验和 token 写入前返回 400。文件路径最长 4096 字节；直接文件写入解码后最多 2MiB，大文件应使用临时上传 URL；文件读取单次最多 2MiB，Agent 返回的文件读取结果按对应 base64 文本预算校验，文件列表 Agent 返回 JSON 最长 2MiB，写入/删除等小结果最长 4KiB；临时下载/上传 URL 签发、配置应用和强制更新都要求目标 Agent 未撤销，撤销后的历史服务器只能继续在管理视图中清理旧记录，不能签发新的公开 bearer URL 或下发新的配置/更新消息；配置 patch 序列化后最多 128KiB。强制更新下载 URL 最长 2048 字节。
