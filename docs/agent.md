...
带参数 bootstrap 是公开端点，`SERVER_URL` 和 `GRPC_SERVER` 只允许指向与该链接请求 `Host` 相同的主机，端口可以不同。公开 query 最长 16KiB，请求 `Host` authority 最长 512 字节，控制面 URL 最长 2048 字节，回显到脚本的 token、Agent 名称和 TLS 参数会做长度校验并拒绝控制字符。跨主机控制面部署请直接下载 GitHub Release 的 `install-agent.sh`，并用环境变量传入 `SERVER_URL` / `GRPC_SERVER`。
