...
| M7 Frontend Complete | All UI flows, permission view, mobile status | ✅ Done. `pnpm lint`, `pnpm build`, and `verify-m7-ui.sh` cover dashboard pages, public status page, permission-aware navigation, terminal UI, file transfer UI, config/update forms, and mobile navigation affordances. |
